...
if(isset($topregion) && ($topserver != '') && ($topclass != '')) {
	...
	$topclasssearch = mysqli_query($conn, "SELECT * FROM `data1` WHERE `region` = '" .$topregion. "' AND `server` = '" .$topserver. "' AND `class` = '" .$classname. "' ORDER BY `total` DESC LIMIT 100");
	...
	$regionserverusers = mysqli_fetch_array(mysqli_query($conn, "SELECT COUNT(DISTINCT `char`) AS `chars` FROM `data1` WHERE `region` = '" .$topregion. "' AND `server` = '" .$topserver. "' AND `class` = '" .$classname. "'"));
	$averageapgained = mysqli_fetch_array(mysqli_query($conn, "SELECT SUM(`total`) AS `sumtotal` FROM `data1` WHERE `region` = '" .$topregion. "' AND `server` = '" .$topserver. "' AND `class` = '" .$classname. "'"));
	$averageapgained = round($averageapgained['sumtotal']/$regionserverusers['chars'], 0);
			
	$averagepercent = mysqli_fetch_array(mysqli_query($conn, "SELECT SUM(`percent`) AS `sumpercent` FROM `data1` WHERE `region` = '" .$topregion. "' AND `server` = '" .$topserver. "' AND `class` = '" .$classname. "'"));
