...
if(isset($topregion) && ($topserver != '') && ($topclass != '')) {
	...
	$regionserverusers = mysqli_fetch_array(mysqli_query($conn, "SELECT COUNT(`char`) AS `chars` FROM `data1` WHERE `region` = '" .$topregion. "' AND `server` = '" .$topserver. "' AND `class` = '" .$classname. "'"));
	...
	<div id="tr">
	<div id="td">' .number_format($regionserverusers['chars']). '</div>
